...
sealed with AES-256-GCM. The key comes from one of two places:

- **Device key (default).** A non-extractable `CryptoKey` generated by WebCrypto and held in
  IndexedDB as a key object, never as bytes. Nothing to remember, nothing to type, and the
  raw key never exists in JavaScript.
- **Passphrase (opt-in).** PBKDF2-SHA256 at 600 000 iterations; the derived key lives only in
  `chrome.storage.session`, which is memory-only and cleared when the browser exits.

Both resolve to a `CryptoKey` before they reach `seal`/`unseal`, so no caller knows or cares
which is in use. The honest comparison between the two is in [SECURITY.md](SECURITY.md).
...
If a passphrase is used it is deliberately independent of the Mozilla password: deriving it
from the account password would mean a password change at Mozilla silently orphans the local
